...
export default function DeleteComputerModal({
  assignedComputers = [],
  selectedBoard,
  onDeleteComputer,
  onClose,
  ...
  const handleDelete = async () => {
    if (!selectedComputer) {
      setError("Por favor selecciona un ordenador para eliminar.");
      return;
    }
      ...
      >
        ...
        >
          <div className="flex flex-row justify-between w-full mb-3 py-2 border-b-2 text-xl font-sansation font-bold">
            <h2>Eliminar un ordenador</h2>
            <h2 className="text-tertiary">Mesa: {selectedBoard?.id}</h2>
          </div>
          ...
          <ul className="space-y-2 max-h-[430px] font-rubik overflow-y-auto">
            {assignedComputers.length > 0 ? (
              ...
              <li className="text-center py-4 font-sansation">
                No hay ordenadores asignados a esta mesa.
              </li>
            )}
          </ul>
          ...
          <div className="w-full flex flex-row justify-between">
            <button
              ...
              disabled={loading}
            >
              <span className="drop-shadow-[2px_2px_4px_rgba(0,0,0,0.7)]">
                Cerrar
              </span>
            </button>
            <button
              onClick={handleDelete}
              disabled={!selectedComputer || loading}
              className={`text-white text-xl font-rubik font-semibold mt-7 px-5 py-[5px] bg-red-600 rounded-md hover:bg-red-700 hover:cursor-pointer transition-colors duration-200 ${
                (!selectedComputer || loading) &&
